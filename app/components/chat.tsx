...
import { initializeMint } from "../filter/constructor";
...
export function Chat() {
  ...
  useEffect(() => {
    async function initMint() {
      const initializedMint = await initializeMint();
      console.log("Initialized mint:", initializedMint); // 检查 mint 实例
      setMint(initializedMint);
    }
  ...
  const onInput = (text: string) => {
    if (mint) {
      let status = mint.filter(text);

      let text1 = status.text;
      setUserInput(text1);
      const n = text1.trim().length;
