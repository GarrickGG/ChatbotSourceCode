...
export function Settings() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      console.log("Decoded user: ", user); // 打印解码后的用户
      ...
      console.log("Stored user: ", sessionStorage.getItem("user")); // 打印存储的用户
    }
  }, []);
  ...
  const handleClick = () => {
    if (!user) {
      window.location.href = "http://www.chatbotsd.tech";
    }
  };
      ...
      <div className={styles["settings"]}>
        <List>
          <ListItem title={Locale.Settings.User}>
            {!user || user.name === "" ? (
              <Link href="http://www.chatbotsd.tech">
                <button>请登录</button>
              </Link>
            ) : (
              <div>{user.name}</div>
          ...
          {user && user.name ? (
            <ListItem title={Locale.Settings.Subscription}>
              {user.isSubscribed ? (
                <div>
                  已订阅，到期时间：
